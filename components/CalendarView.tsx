...
const CalendarView: React.FC<CalendarViewProps> = ({ remocoes }) => {
  // Simplified Logic: Show current month (static October 2023 based on mock or dynamic today)
    ...
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 h-[calc(100vh-140px)] flex flex-col animate-in fade-in">
      <div className="p-4 border-b border-slate-200 flex justify-between items-center">
        ...
        <div className="flex gap-2">
          <button className="p-2 hover:bg-slate-100 rounded-full text-slate-500"><ChevronLeft className="w-5 h-5" /></button>
          <button className="p-2 hover:bg-slate-100 rounded-full text-slate-500"><ChevronRight className="w-5 h-5" /></button>
        </div>
      </div>
            ...
            return (
              <div key={day} className={`border-r border-b border-slate-100 p-2 min-h-[100px] hover:bg-slate-50 transition-colors ${isToday ? 'bg-indigo-50/30' : ''}`}>
                ...
                <div className="space-y-1">
                  {events.map(ev => (
                    <div key={ev.id} className="text-xs bg-indigo-100 text-indigo-700 p-1.5 rounded border border-indigo-200 truncate cursor-pointer hover:bg-indigo-200 transition-colors" title={`${ev.horaOrigem} - ${ev.paciente}`}>
